...
2. **Is the grant made by the containing app honoured for the embedded appex, or must the appex prompt separately?** Determines whether the companion app can do the onboarding (good) or whether the user gets an invisible prompt while away (fatal, needs a workaround).
